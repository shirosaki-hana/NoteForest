...
export function useNoteDelete({
  ...
  const handleDeleteClick = (note: Note, event: React.MouseEvent) => {
    event.stopPropagation(); // 노트 선택 이벤트 방지
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (response.success) {
        // 삭제된 노트가 현재 선택된 노트라면 선택 해제
        if (selectedNoteId === noteToDelete.uuid) {
          onNoteSelect?.('');
        }

        // 백엔드와 동기화를 위해 노트 목록 다시 로드
        await onDeleteSuccess();

        showMessage('노트가 성공적으로 삭제되었습니다.', 'success');
      } else {
        showMessage(response.error || '노트 삭제에 실패했습니다.', 'error');
      }
    } catch (err) {
