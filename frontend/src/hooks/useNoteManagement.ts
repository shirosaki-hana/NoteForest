...
export interface UseNoteManagementReturn {
  ...
  handleTagsChange: (event: React.SyntheticEvent | null, newValue: string[]) => void;
...
export function useNoteManagement(
  ...
  const debouncedSaveDraft = useDebounce(
    (noteId: string, title: string, tags: string[], content: string) => {
  ...
  useEffect(() => {
    if (!selectedNoteId) {
      ...
      if (draftKeys.length > 0) {
        ...
        let latestDraft:
          | { noteId: string; title: string; tags: string[]; content: string; lastModified: number }
          | undefined;
  ...
  const handleTagsChange = (_event: React.SyntheticEvent | null, newValue: string[]) => {
